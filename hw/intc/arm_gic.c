...
printf("%s: mask(0x%x), cm(0x%x), irq+i = %d, !s->irq_state[irq + i].group(0x%x), !secure(0x%x)\n", __func__, mask, cm, irq+i, s->security_extn, !attrs.secure);
                if (s->security_extn && !attrs.secure &&
                    !GIC_TEST_GROUP(irq + i, 1 << cpu)) {
                    continue; /* Ignore Non-secure access of Group0 IRQ */
                }
printf("%s: Enable irq(%d)\n", __func__, irq+i);
