...
bool is_rdwr_access_supported(AccessType access,
                                     uint64_t addr, unsigned size)
{
    /* This is a quicker way then doing a search in the table. */
    if (size <= 8) {
        uint8_t byte_addr = addr & 0x7;
        return size == 1 ||
               (size == 2 && byte_addr % size == 0) ||
               (byte_addr == 0 || byte_addr == 8 - size);
    } else { /* size > 8 */
        return size % 8 == 0 && (addr & 0x7) == 0x0 &&
               is_rdwr_len_supported(access, size);
    }
}
...
void pack_body(uint8_t *buf, unsigned size, unsigned *pos, const RioPkt *pkt)
{
    ...
    switch (pkt->ftype) {
      ...
      case RIO_FTYPE_RESP:
        ...
        break;
      default:
        qemu_log("RIO: ERROR: packet ftype %x not supported\n", pkt->ftype);
        abort();
    }
}
...
#define STRINGIFIED_SIZE(dwords) \
    (dwords * (8 /* bytes */ * 2 /* chars */ + 2 /* spaces */))

static int stringify_buf(char *s, size_t size, uint8_t *buf, size_t len)
{
    int p = 0, n;
    for (int b = 0; b < len; ++b) {
        n = snprintf(s + p, size - p, (b % 4 == 0) ? " %02x" : "%02x", buf[b]);
        if (n < 0 || n >= size - p) {
            qemu_log("RIO: WARNING: failed to stringify buf\n");
            if (size > 0) {
                s[0] = '\0';
            }
            return -1;
...
{
#define DWORDS_TO_PRINT 4  /* print only first several dwords */
    char pkt_str[STRINGIFIED_SIZE(DWORDS_TO_PRINT)];
    stringify_buf(pkt_str, sizeof(pkt_str), pkt_buf, MIN(len, DWORDS_TO_PRINT * 8));
    qemu_log("RIO: %s (size %u): %s\n", prefix, len, pkt_str);
#undef DWORDS_TO_PRINT
}
